(function () {
  ...
  var GUIDE = {
    ...
    9: {
      glossary: [
        { term: "Framework", desc: "Grundgerüst für Anwendungsentwicklung — Nuxt baut auf Vue.js auf und liefert Routing, Rendering und Build-Integration." },
        { term: "Kernframework", desc: "Der Nuxt-Kern ohne Erweiterungsmodule — im CLI-Workflow separat vom Modul-Upgrade bearbeitbar." },
  ...
  Object.keys(GUIDE)
    ...
    .forEach(function (key) {
      (GUIDE[key].glossary || []).forEach(function (item) {
        var existing = merged.get(item.term);
        if (!existing || item.desc.length > existing.desc.length) {
          merged.set(item.term, item);
        }
      });
    });

  window.PRESENTER_GUIDE = GUIDE;
  window.PRESENTER_ALL_GLOSSARY = Array.from(merged.values()).sort(function (a, b) {
